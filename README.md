...
| `--replay-cache-dir` / `ALPACA_MOCK_REPLAY_CACHE_DIR` | Optional shared file cache for successful replay `bars` and historical quote-window responses. Use the same path across test mock servers to remove upstream network/response drift from repeated runs. |
