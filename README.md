...
| Item | Role |
|------|------|
| `--alpaca-end-date` / `ALPACA_MOCK_ALPACA_END_DATE` | Inclusive replay end day (weekdays only). Replay clock skips overnight/weekends between start and end. |
| `--alpaca-time` / `ALPACA_MOCK_ALPACA_TIME` | Where the **replay clock** starts on the first session day (default `09:30`). Configuration, not a price override. |
...
| `--replay-step-seconds` / `ALPACA_MOCK_REPLAY_STEP_SECONDS` | Optional replay-time grid. `30` means replay now advances on 30-second boundaries, reducing small wall-clock/polling jitter across repeated runs. |
| `--replay-cache-dir` / `ALPACA_MOCK_REPLAY_CACHE_DIR` | Shared file cache for successful replay `bars` and historical quote-window responses. Default: `/tmp/alpaca_mock_replay_cache`; set to `off` to disable. Use the same path across test mock servers to remove upstream network/response drift from repeated runs. |
...
python mock_server.py --alpaca-date 2026-05-13 --alpaca-time 09:35 --replay-speed 3 --access-log
